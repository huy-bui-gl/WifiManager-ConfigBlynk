 ...
 **************************************************************/
  ...
  server->on(String(F("/blynk")).c_str(), std::bind(&WiFiManager::handleBlynkSave, this));
...
//*                                                                                            *
//**********************************************************************************************
void WiFiManager::handleBlynkSave() {
  DEBUG_WM(F(" blynk Save"));

  String _token = server->arg("token");
  if(_token.length() > 0 && _token.length() <= MAX_TOKEN ){
    if(_token != "clear" && _token != "Clear"){
      saveToken(_token);
      }else{
        clearToken();
      }
    ...
    page += _customHeadElement;
    page += FPSTR(HTTP_HEADER_END);
    page+= FPSTR(HTTP_ICON_BLINK);
    page+= FPSTR(HTTP_BLYNK_PAGE);
    page += FPSTR(HTTP_END);

    page.replace("{token-blynk}",tokenW);
    page.replace("{max-value}",String(MAX_TOKEN));
    page.replace("{max-leng}",String(MAX_TOKEN));
...
{
  EEPROMclass      EppROM_Object;
  String pass_r =  EppROM_Object.read(0,MAX_TOKEN);
  return pass_r;
}


// save token blynk
void WiFiManager::saveToken(String token_w)
{
  EEPROMclass      EppROM_Object; 
  EppROM_Object.write(token_w,0,MAX_TOKEN);
}

// save token blynk
void WiFiManager::clearToken()
{
  EEPROMclass      EppROM_Object; 
  EppROM_Object.clear(0,MAX_TOKEN);
}
...
{   
  ...
  if(data.length() <= endA - beginA ){
    clear(beginA, endA);
    for (int i = beginA; i <= beginA + data.length(); i++) 
    {   
    ...
    if (EEPROM.commit()) { 
      // Serial.println(F("[EEPRom][write] Ghi thành công "));
      } else {
       // Serial.println(F("[EEPRom][write] Ghi thất bại "));
     }
   }
 }
